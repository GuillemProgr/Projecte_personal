...
import MorosidadForm from "@/components/modulos/morosidad-form";
...
export default function MorosidadPage() {
  ...
  return (
    <div className="min-h-full">
      <div className="px-10 pt-8 pb-8" style={{ background: "linear-gradient(135deg,#1a0a0a,#1a0d0d)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="flex items-center gap-1.5 text-xs mb-4" style={{ color: "rgba(255,255,255,0.4)" }}>
          ...
          <ChevronRight className="w-3 h-3" />
          <span style={{ color: "rgba(255,255,255,0.7)" }}>Morosidad</span>
        </div>
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: "rgba(239,68,68,0.2)", border: "1px solid rgba(239,68,68,0.3)" }}>
            <AlertTriangle className="w-5 h-5" style={{ color: "#F87171" }} />
          </div>
          <div>
            <h1 className="text-xl font-bold text-white">Carta de Morosidad</h1>
            <p className="text-xs" style={{ color: "rgba(255,255,255,0.4)" }}>Reclamación formal con referencias al art. 9 y 21 LPH</p>
          </div>
        </div>
      </div>
      <div className="px-10 py-8 max-w-3xl">
        {!docId ? (
          <div className="rounded-2xl bg-card border border-border/60 p-7">
            <MorosidadForm comunidadId={id} onGenerado={(i, c) => { setDocId(i); setContenido(c); }} />
          </div>
        ) : (
          <div className="space-y-4">
            <div className="flex items-center gap-2.5 rounded-xl px-4 py-3 text-sm font-medium" style={{ background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.2)", color: "#16a34a" }}>
              ...
              Carta generada. Revísala y exporta a Word.
            </div>
            ...
            <button onClick={() => { setDocId(null); setContenido(""); }} className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors mt-2">
              <ArrowLeft className="w-3.5 h-3.5" /> Generar otra carta
            </button>
          </div>
        )}
      </div>
