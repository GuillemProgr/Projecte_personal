...
import ConvocatoriaForm from "@/components/modulos/convocatoria-form";
import DocumentoEditor from "@/components/documento-editor";
import { CalendarDays, ArrowLeft, CheckCircle2, ChevronRight } from "lucide-react";
...
export default function ConvocatoriasPage() {
  ...
  return (
    <div className="min-h-full">
      <div className="px-10 pt-8 pb-8" style={{ background: "linear-gradient(135deg,#0e0a1a,#110d1f)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="flex items-center gap-1.5 text-xs mb-4" style={{ color: "rgba(255,255,255,0.4)" }}>
          ...
          <ChevronRight className="w-3 h-3" />
          <span style={{ color: "rgba(255,255,255,0.7)" }}>Convocatorias</span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">Convocatoria de Junta</h1>
            <p className="text-xs" style={{ color: "rgba(255,255,255,0.4)" }}>Primera y segunda convocatoria (art. 16 LPH)</p>
          </div>
        </div>
      </div>
      <div className="px-10 py-8 max-w-3xl">
        {!docId ? (
          <div className="rounded-2xl bg-card border border-border/60 p-7">
            <ConvocatoriaForm comunidadId={id} onGenerado={(i, c) => { setDocId(i); setContenido(c); }} />
          </div>
        ) : (
          <div className="space-y-4">
            <div className="flex items-center gap-2.5 rounded-xl px-4 py-3 text-sm font-medium" style={{ background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.2)", color: "#16a34a" }}>
              ...
              Convocatoria generada. Revísala y exporta a Word.
            </div>
            ...
            <button onClick={() => { setDocId(null); setContenido(""); }} className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors mt-2">
              <ArrowLeft className="w-3.5 h-3.5" /> Nueva convocatoria
            </button>
          </div>
        )}
      </div>
